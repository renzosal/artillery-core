...
function SocketIoEngine(script) {
  ...
  if (script.config.socketio) {
    ...
    if (script.config.socketio.namespace) {
      this.namespace = script.config.socketio.namespace;
    }

    if (script.config.socketio.authenticate) {
      this.authenticate = script.config.socketio.authenticate;
    }
...
SocketIoEngine.prototype.step = function (requestSpec, ee) {
  ...
  function preStep(context, callback){
    ...
    requestSpec.emit.namespace = template(requestSpec.emit.namespace, context) || self.namespace || '/';
  ...
  if(!context.sockets[namespace]) {
    ...
    context.sockets[namespace].on('connect', function(){
      if(self.authenticate) {
        let token = template(self.authenticate, context);
        debug(token);
        context.sockets[namespace].emit('authenticate', {token: token });
      } else {
        cb(null, context.sockets[namespace]);
      }
    });

    context.sockets[namespace].on('authenticated', function() {
